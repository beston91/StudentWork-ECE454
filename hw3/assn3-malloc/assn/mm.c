...
printf("coalesce\n");
    ...
    if((void*)PREV_BLKP(bp) > mem_heap_hi() || (void*)PREV_BLKP(bp) < mem_heap_lo()){
			prev_alloc = 1;
    }
    if((void*)NEXT_BLKP(bp) > mem_heap_hi() || (void*)NEXT_BLKP(bp) < mem_heap_lo()){
				next_alloc = 1;
...
printf("CC\n");
    ...
    {   /* Case 2 */
        ...
        */
                    printf("2 %x %x %x\n", (void*)NEXT_BLKP(bp), mem_heap_hi(), mem_heap_lo());

			size_t size_bk = size;
      list_entry_t* next = NULL;
      list_entry_t* pt = free_list[list_hash(GET_SIZE(HDRP(NEXT_BLKP(bp))))];
			for(; pt!=NULL; pt= pt->prev){
				if((void*)pt == NEXT_BLKP(bp)){
					next = pt;
					break;
				}
			} 
      ...
      PUT(FTRP(bp), PACK(size, 0));
             if(list_hash(size_bk) != list_hash(size)){
                pt = free_list[list_hash(GET_SIZE(HDRP(bp)))];
                for(; pt!=NULL; pt= pt->prev){
                    if((void*)pt == bp){
    ...
    {   /* Case 3 */
        ...
        */
        ...
        if(list_hash(size_bk) != list_hash(size)){
            pt = free_list[list_hash(GET_SIZE(HDRP(PREV_BLKP(bp))))];
            for(; pt!=NULL; pt= pt->prev){
                if((void*)pt == NEXT_BLKP(bp)){
                    prev = pt;
                    break;
...
int mm_check(void) {
    ...
    if(GET_ALLOC(HDRP(heap_pt)) == 0 && GET_SIZE(HDRP(heap_pt)) != 0){
    	free_block_count2 ++;
    } 
