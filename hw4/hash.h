...
pthread_mutex_t list_mutex[16384]; 
...
template<class Ele, class Keytype> class hash {
 ...
 public:
  ...
  void reset();
  void cleanup();
  void this_list_lock(Keytype the_key);
  void this_list_unlock(Keytype the_key);
};
...
template<class Ele, class Keytype> 
void 
hash<Ele,Keytype>::this_list_lock(Keytype the_key){
  pthread_mutex_lock(&list_mutex[HASH_INDEX(the_key,my_size_mask)]);
}

template<class Ele, class Keytype> 
void 
hash<Ele,Keytype>::this_list_unlock(Keytype the_key){
  pthread_mutex_unlock(&list_mutex[HASH_INDEX(the_key,my_size_mask)]);
}
